...
def process_user_input(user_text, df):
    ...
    and extract data or generate a reply. Includes rate limit handling.
    """
    ...
    if not MY_API_KEY or MY_API_KEY == "PASTE_YOUR_API_KEY_HERE":
        return {"intent": "chat", "chat_reply": "Error: API Key is not configured. Please check app settings."}
...
def main():
    ...
    # --- 主應用程式頁面 (已登入) ---
        ...
        with st.sidebar:
            ...
            # 登出按鈕
            if st.button("Logout"):
                st.session_state.update({"logged_in": False, "messages": []}) # 登出時清空會話狀態
                st.rerun() 
